...
void alloc_env(glass_env* env) {
	...
	memset(env->c_lookup, 0, MAX_PROGRAM * sizeof (token_t));
}

void add_class(glass_env* env, char* name) {
	// adds a class to the env c_lookup
	if (find_name(env->names, name) >= 0) {
		parse_error("cannot overwrite existing class");
	}
...
void add_class_func(glass_env* env, char* c_name, char* f_name) {
	...
	// this should never happen
	if (!c_idx) parse_error("couldn't find class name");

	// pull out the relevant entry in f_lookup
	int* f_name_idxs = env->f_lookup[c_idx];
	int empty_i = 0;
	while(empty_i && empty_i < MAX_FUNCS) empty_i++;
...
void init_env(glass_env* env) {
	...
	// establish 
	char* standard_names[] = {"A", "S", "V", "O", "I", "M", NULL};

	// establish orders of the standard 
	...
	for (int i = 0; stds[i]; i++) {
		for (int j = 0; stds[i][j]; j++) {
			add_class_func(env, standard_names[i], stds[i][j]);
		}
	}
}
