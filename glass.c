...
// Data structures for program execution
...
void check_par(char* start, char a, char b, char end) {
	...
	for (int i = 0; start[i] != end; i++) {
		if (!start[i]) glass_error("overran string in check_par");
		if (start[i] == a) ps++;
		else if (start[i] == b) ps--;
		if (ps < 0) glass_error("mismatched");
	}
	if (ps != 0) glass_error("mismatched");
}
...
void read_name(char* buff, char* start, int lim) {
	...
	if (*start == '(') {
		start++;
		for (int i = 0; i != ')'; i++) {
			if (!isalpha(start[i])) glass_error("bad name");
			if (i >= (lim - 2)) glass_error("name too long");
			buff[i] = start[i];
			buff[i + 1] = 0;
		}
	}
	else {
		if (!isalpha(*start)) glass_error("bad name");
		buff[0] = *start;
		buff[1] = 0;
	}
}
...
char* read_clean(char* loc) {
	...
	if (f) {
		...
		len = ftell (f);
  		...
  		else glass_error("malloc error in read_clean");

  		if (buff) fread(buff, sizeof (char), len, f);
  		fclose(f);
	}

	if (!buff) glass_error("couldn't read file");
...
named_val build_func(char* start) {
	...
	res.v.name = (char*) malloc(64);
	read_name(res.v.name, start, 64);
